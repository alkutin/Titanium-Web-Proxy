...
namespace ERemoteProxy.ServiceLibrary
{
    public class ERemoteHandler
    {
        ...
        public ERemoteHandler()
        {
            _encoder = new Proxy.Encoding.Encoder(Encoding.ASCII.GetBytes(ConfigurationSingleton.Instance.Key), Encoding.ASCII.GetBytes(ConfigurationSingleton.Instance.Vector));
        }
        ...
        public byte[] Get(Guid key, bool body, long position, long size)
        {
            ResponseTuple info;
            long readPosition = 0;
                ...
                if (body)
                {
                    ...
                    if (position == 0 && size == 0)
                    {
                        Debug.WriteLine("Requested whole body for " +
                            info.RequestHeader.RequestAsUri.AbsoluteUri);
                    ...
                    else
                    {
                        ...
                        while (!readComplete)
                        {
                            ...
                            readSize = bodyStream.Read(buffer, 0, (int)size);
